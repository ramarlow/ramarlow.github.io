---
title: Rover Display Platform
layout: single
permalink: /projects/platform
toc: false
author_profile: true
header:
    teaser: /assets/images/display-finished.PNG
excerpt: "Wisconsin Robotics decided to retire their long-serving rover at the end of the 2024 URC season to make way for a full redesign, so we got permission to display the old vehicle in a corner of the ME building, for which I helped build a display platform."
classes: wide
---
Wisconsin Robotics decided to retire their long-serving rover at the end of the 2024 URC season to make way for a full redesign, so we got permission to display the old vehicle in a corner of the ME building, for which I helped build a small display platform.